/**
 * Sync Complete Rifle Caliber Data to Algolia
 * Update Algolia index with complete rifle caliber data
 */

import { Pool } from 'pg';

async function syncRifleCaliberComplete() {
  console.log('📊 Syncing Complete Rifle Caliber Data to Algolia...');
  ...
  try {
    ...
    const algoliaUpdates = riflesWithCaliber.rows.map(rifle => ({
      objectID: rifle.sku,
...
// Run the sync
syncRifleCaliberComplete().catch(console.error);